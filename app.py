...
# TAB 2: WORD2VEC SEMANTIC MAPS (WITH INTEGRATED REPLACE ACTION)
with tabs[1]:
    ...
    if not st.session_state.processing_done:
        ...
        if st.session_state.search_term:
            ...
            if st.session_state.wv_model and t_clean in st.session_state.wv_model.wv:
                ...
                # Render interactive selectable data viewer matching desktop table model selections
                ...
                # Safe resolution structure for row indexes matching multiple streamlit configurations
                selected_indices = []
                ...
                if selected_indices:
                    ...
                    # Original Desktop Replace Action Ported to Native HTML Layout Engine
                    if st.button(f"🔄 Replace Selected Terms with '{t_clean}'", type="primary"):
                        df = st.session_state.main_data.copy()
                        
                        # Generate precise whole-word matching regular expression bounds
                        ...
                        with st.spinner("Modifying underlying text matrices and re-compiling embeddings..."):
                            ...
                            # Re-train models to refresh semantic weights map across modified records
                            st.session_state.wv_model = Word2Vec(sentences=st.session_state.clean_sentences, min_count=20, vector_size=200, window=3, sg=1)
                            st.session_state.main_data = df
                            
                        st.success(f"Successfully replaced {', '.join(selected_words)} with '{t_clean}' across data columns!")
                        st.keyboard_listener = None
                        st.rerun()
                else:

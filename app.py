...
# ==========================================
# 1. Page Configuration & Header Presentation
# ==========================================
st.set_page_config(page_title="KeyText Version 0.22", layout="wide")

# App Header Layout Matrix
st.title("🔑 KeyText Version 0.22")

# Native Dialog Architecture for Citation Modal Component
@st.dialog("📚 Citation Information")
def show_citation_dialog():
    bibtex_string = """@software{maulana_keytext_2026,
    address = {Jakarta},
    title = {{KeyText Web Version 0.22}},
    type = {Computer software},
    url = {https://keytext.streamlit.app},
    author = {Maulana, Ikbal},
    year = {2026},
    institution = {National Research and Innovation Agency (BRIN)},
}"""
    st.write("To reference this application framework inside your academic publications, please utilize the following BibTeX reference notation:")
    st.code(bibtex_string, language="bibtex", line_numbers=False)
    if st.button("❌ Close Panel", use_container_width=True):
        st.rerun()

# Sub-Header Meta Row Construction Layout
meta_col1, meta_col2 = st.columns([1, 6])
with meta_col1:
    if st.button("📄 Cite app", type="secondary", help="View BibTeX reference block parameters."):
        show_citation_dialog()
with meta_col2:
    st.markdown("<p style='padding-top: 6px; color: gray; font-weight: 500;'>Copyright ©2026 Ikbal Maulana</p>", unsafe_allow_html=True)
...
if "dataset_has_changes" not in st.session_state:
    st.session_state.dataset_has_changes = False
if "kwic_just_replaced" not in st.session_state:
    st.session_state.kwic_just_replaced = False

# Track the last seen file list to automate background reloading without a button click
...
for key in ["kw1", "kw2", "kw3", "lbl1", "lbl2", "lbl3", "search_term", "kwic_term", "substitution_term"]:
...
# Callback to clear dirty flag state after download completion
def clear_changes_flag():
    st.session_state.dataset_has_changes = False

# Callbacks to reset button visibility state when user typing occurs
def on_kwic_search_change():
    st.session_state.kwic_just_replaced = False

def on_substitution_change():
    st.session_state.kwic_just_replaced = False

# ==========================================
# 3. Sidebar Configuration & Automated File Loader
# ==========================================
with st.sidebar:
    ...
    # Calculate fingerprint of active upload pool to run automated loading
    current_files_hash = None
    if uploaded_files:
        current_files_hash = "".join([f"{f.name}_{f.size}" for f in uploaded_files]) + f"_{txt_delimiter}"

    # Auto-load logic triggered reactively when the selected file array shifts
    ...
    st.markdown("---")
    st.subheader("📥 Export Dataset Workspace")
    
    current_date_str = datetime.now().strftime("%Y-%m-%d")
    export_filename = f"{st.session_state.original_filename}_{current_date_str}.csv"
    
    if not st.session_state.main_data.empty and st.session_state.dataset_has_changes:
        ...
        st.download_button(
            ...
            on_click=clear_changes_flag
        )
    else:
        st.button("💾 Download Modified Dataset", disabled=True, use_container_width=True, help="Enabled automatically after changes are processed.")

# ==========================================
# 4. Modular Workspace Environment Tabs
# ==========================================
tabs = st.tabs([
    "📊 Dataset", 
...
# TAB 1: DATASET CONTROL PANEL & EMBEDDING INITIATOR
with tabs[0]:
    if st.session_state.processing_done:
        st.success("🔒 Dataset processing complete! This tab configuration interface is locked down.")
        st.info("To make additional adjustment variations or choose alternative text vectors, reload your target data files via the sidebar upload portal container.")
    else:
        if st.session_state.main_data.empty:
            st.info("Awaiting structural dataset input files from sidebar file uploader selection.")
        else:
            columns_list = ["Select"] + list(st.session_state.main_data.columns)
            ...
            st.session_state.text_col_choice = text_col_choice
            st.session_state.date_col_choice = date_col_choice
            ...
            st.markdown("---")
            st.subheader("📋 Active Loaded Dataset View Matrix")
            st.dataframe(st.session_state.main_data, width="stretch")

# Resolve text column indices securely across modular processing boundaries
text_col_choice = st.session_state.get("text_col_choice", "Text")
date_col_choice = st.session_state.get("date_col_choice", "Date")

# TAB 2: WORD2VEC SEMANTIC MAPS
with tabs[1]:
    st.header("🔍 Neural Embedding Distance Vector Maps")
    if not st.session_state.processing_done:
        st.info("Run core parameter execution inside the Dataset tab to unlock word embeddings mapping vectors.")
    else:
                ...
                if selected_indices:
                    ...
                    if st.button(f"🔄 Replace Selected Terms with '{t_clean}'", type="primary", key="w2v_replace_btn"):
                        ...
                        with st.spinner("Modifying underlying text matrices and re-compiling embeddings..."):
                            st.session_state.main_data = df
                            recompile_pipeline_matrices()
                            st.session_state.dataset_has_changes = True
...
# TAB 3: KEY WORD IN CONTEXT (KWIC) EXPLORER (WITH DYNAMIC REPLACEMENT VISIBILITY CONTROLS)
with tabs[2]:
    st.header("📖 Contextual Word Frame Windows")
    if not st.session_state.processing_done:
        st.info("Run core parameter execution inside the Dataset tab to extract local contexts.")
    else:
        # Integrated change monitoring listeners to clear the hidden state upon new user text entry
        kwic_input_raw = st.text_input(
            "Context Focus Targeted Node String (supports phrases and wildcards, e.g., merah putih, ber*, *nya)", 
            value=st.session_state.kwic_term,
            key="kwic_term_widget",
            on_change=on_kwic_search_change
        )
        ...
        with kc2:
            replacement_word_input = st.text_input(
                "Substitution Word", 
                value=st.session_state.substitution_term,
                key="substitution_term_widget",
                on_change=on_substitution_change
            )
            st.session_state.substitution_term = replacement_word_input
        ...
        if cleaned_kwic_input:
            ...
            # Button is rendered ONLY if search is not empty, substitution is provided, and replacement wasn't just executed
            if replacement_word_input.strip() and not st.session_state.kwic_just_replaced:
                ...
                if st.button(f"🔄 Replace all matching occurrences of '{cleaned_kwic_input}' with '{rep_word_clean}'", type="primary"):
                    ...
                    with st.spinner("Executing structural context replacement and re-indexing corpora..."):
                        ...
                        # Toggle tracking variables to instantly trigger button disappearance on render
                        st.session_state.kwic_just_replaced = True
                        st.session_state.dataset_has_changes = True
...
with tabs[3]:
    ...
    if not st.session_state.processing_done:
        st.info("Run core parameter execution inside the Dataset tab to activate statistical calculators.")
    else:
...
with tabs[4]:
    ...
    if not st.session_state.processing_done:
        st.info("Run core parameter execution inside the Dataset tab to populate multi-gram configurations tables.")
    else:
...
with tabs[5]:
    ...
    if not st.session_state.processing_done:
        st.info("Run core parameter execution inside the Dataset tab to unlock complex relational calculations matrices.")
    else:

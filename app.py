...
# TAB 3: KEY WORD IN CONTEXT (KWIC) EXPLORER (WITH PHRASE DETECTOR & REPLACE OPTIONS)
with tabs[2]:
    st.header("📖 Contextual Word Frame Windows")
    if not st.session_state.processing_done:
        st.info("Run core parameter execution above to extract local contexts.")
    else:
        kwic_input_raw = st.text_input("Context Focus Targeted Node String", value=st.session_state.kwic_term)
        
        # -------------------------------------------------------------
        # Continuous Multi-Word Phrase Detector & Suggestion Interface
        # -------------------------------------------------------------
        cleaned_kwic_input = kwic_input_raw.strip()
        words_in_input = cleaned_kwic_input.split()
        
        if len(words_in_input) > 1:
            suggested_joined_token = "_".join(words_in_input)
            st.warning(f"⚠️ Multi-word target detected. For text analysis pipelines, joined tokens are recommended.")
            if st.button(f"🔗 Connect phrase with underscores: '{suggested_joined_token}'", type="secondary"):
                st.session_state.kwic_term = suggested_joined_token
                st.rerun()
        else:
        ...
        # Trigger dynamic replacement logic if a target substitution token is specified
        if target_kwic and replacement_word_input.strip():
            rep_word_clean = replacement_word_input.strip().lower()
            
            # Phrase check for the destination word input box
            if len(rep_word_clean.split()) > 1:
                st.error("⚠️ Substitution targets must consist of a singular connected token.")
            else:
                if st.button(f"🔄 Replace all whole-word occurrences of '{target_kwic}' with '{rep_word_clean}'", type="primary"):
                    df = st.session_state.main_data.copy()
                    pattern = r'\b{}\b'.format(re.escape(target_kwic))
                    df['SelectedColumn'] = df['SelectedColumn'].astype(str).str.replace(pattern, rep_word_clean, regex=True)
        ...
        if target_kwic:
            ...
            if records:
                st.subheader(f"Active Context Windows Frame for: '{target_kwic}'")
                st.dataframe(pd.DataFrame(records), width="stretch")
            else:
                st.warning(f"No active window matching trajectories found for: '{target_kwic}'")
